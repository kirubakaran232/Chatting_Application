      ...
      {stories.map((story) => (
        <button key={story._id} onClick={() => setActive(story)} className="group relative h-12 w-12 shrink-0 overflow-hidden rounded-2xl border-2 border-pink-400 sm:h-16 sm:w-16">
          ...
          {story.author?._id === user._id && (
            <button onClick={(e) => { e.stopPropagation(); deleteStory(story._id); }} className="absolute right-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-black/60 text-white opacity-100 sm:opacity-0 sm:group-hover:opacity-100" title="Delete story">
              <Trash2 size={12} />
            </button>
          )}
        </button>
